...
Note that the kernel or NIC may strip VLAN tags before the frame reaches an `AF_PACKET` socket (VLAN offload, or scanning a `vlanN` sub-interface rather than the trunk parent). `ETH_P_ALL` is what lets *unstripped* tagged frames through; it cannot restore a tag that was already removed.
