...
- Install a **filter program** (`BIOCSETF`) so untagged ARP (`EtherType 0x0806`), IEEE 802.1Q-tagged ARP (`0x8100` then inner `0x0806`), RFC 1042 LLC/SNAP ARP (IEEE 802.3 length then `AA AA 03 00 00 00 08 06`), and 802.1Q-plus-SNAP ARP frames are delivered, and disable **`BIOCSSEESENT`** so the device does not echo back the requests the tool broadcasts. Together these match the effect of a Linux packet socket bound to `ETH_P_ARP`, plus tagged and SNAP ARP that would otherwise be dropped at offset 12. When **`--vlan`** is set, transmitted frames include a single IEEE 802.1Q tag. When **`--llc`** is set, transmitted frames use RFC 1042 SNAP. Receive filtering always includes those framings so SNAP replies are not dropped even on an Ethernet II send. Well-formed ARP that is not a reply is ignored in userspace without a malformation warning.
